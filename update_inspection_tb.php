...
$query = "SELECT HORSE FROM tsales WHERE HORSE = ? LIMIT 1";
...
$insertQuery = "INSERT IGNORE INTO horse_inspection_tb (horse) VALUES (?)";
...
$query = "SELECT * FROM horse_inspection_tb WHERE horse = ? LIMIT 1";
$stmt = $mysqli->prepare($query);
$stmt->bind_param('s', $found_horse_name);
$stmt->execute();
...
$updateQuery = "UPDATE horse_inspection_tb SET `$field` = ? WHERE horse = ?";
